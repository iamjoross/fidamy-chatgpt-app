...
- `app.py` wires FastMCP, handlers, and the Starlette app.
- `widgets.py` defines widget metadata and loads the built HTML assets.
- `schemas.py` holds the tool input model and JSON schema.
- `main.py` remains the runnable entrypoint.
- `server.py` is kept as a small compatibility shim for older imports.
